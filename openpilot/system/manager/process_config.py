...
procs += [
  ...
  # GS450h: chestnut の GPU ハングで落ちたら再起動させる (upstream 既定だと 1 走行まるごと無モデルのまま engage できてしまう)。
  # 再起動で am_usb の flock と tinygrad 状態が消えるので小モデルへ降格せず big のまま復帰できる (ManagerProcess.reap_if_crashed)
