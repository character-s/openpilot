...
#   ① ランプを [1.0, 2.5] -> [0.3, 1.0] へ短縮。旧値では **ramp=1 の滞在率が 0-6% (>61km/h でも
#      15-30%)、実効 trim の中央値がほぼ +0.0%** = テーブルが滞在時間の 98% に届いていなかった。
#      2.5s 未満で抜ける曲がり角には事実上 1 bit も掛かっていない。
#      ⇒ 短縮で届く率 1.8% -> 3.5%、>61km/h の overshoot -27.8%、fulfill p50 103.7 -> 100.4%。
#      ⚠ [0.0, 0.3] まで詰めると fulfill が 98.1% = 足りない側へ行き過ぎる (shadow で確認済)。
...
SETTLED_RAMP_X = [0.3, 1.0]    # 同符号カーブの継続時間 [s] (09-20: [1.0, 2.5] から短縮)
...
SETTLED_TRIM_TBL = [
    ...
    [0.000, 0.000, 0.000, -0.034],            # 43km/h  実測 g=0.988/0.967/0.935/0.938 (同上)
