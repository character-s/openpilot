...
# GS450h PLN-1_1: a flat 2.0 nagged constantly (median 9 km/h below the speed the driver took the same
# corner at) and a flat 3.0 gave up the corners that matter, so the limit depends on how sharp the turn
# ahead looks. The top end is 2.6 rather than the comfort-book 2.0 on purpose: the corners that land there
# are long chains of bends where the driver has already braced for the lateral load, and a flat comfort
# limit there is what made the feature feel like it was fighting the driver.
# Tuned by replaying logged routes with archive/probes/_scc_replay.py.
...
# GS450h PLN-1_1: stock [-0.2, -1.] x [1.3, 3.] only asked for -0.2..-0.45 in the 1.3-2.0 band that
# right-angle junctions produce, so the car coasted into the turn and the driver had to brake. Together
# with the curvature-dependent limit above, such junctions still end up slacker than stock - a deliberate
# trade: 4 s out they predict only ~1.6 m/s2, indistinguishable from a gentle bend, and buying them back
